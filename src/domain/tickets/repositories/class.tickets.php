...
namespace leantime\domain\repositories {
    ...
    class tickets
    {
        ...
        public function getChats($id, $id_ticket)
        {
            $query = "SELECT * FROM `zp_chats` WHERE id_user = :id_user AND id_ticket = :id_ticket";
            $stmn = $this->db->database->prepare($query);
            $stmn->bindValue(':id_user', $id, PDO::PARAM_INT);
            $stmn->bindValue(':id_ticket', $id_ticket, PDO::PARAM_INT);
            $stmn->execute();
            $values = $stmn->fetchObject('\leantime\domain\models\tickets');
            $stmn->closeCursor();
